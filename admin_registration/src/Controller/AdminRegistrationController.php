...
namespace Drupal\admin_registration\Controller;

use Drupal\Component\Utility\Crypt;
use Drupal\Component\Utility\Xss;
use Drupal\Core\Controller\ControllerBase;
use Drupal\Core\Datetime\DateFormatterInterface;
use Drupal\admin_registration\Form\AdminPasswordResetForm;
...
class AdminRegistrationController extends ControllerBase {
  ...
  public function getResetPassForm(Request $request, $uid) {
    ...
    return $this->formBuilder()->getForm(AdminPasswordResetForm::class, $user, $expiration_date, $timestamp, $hash);
  }
  ...
  public function userEdit(UserInterface $user) {
    if ($user->hasRole('site_administrator') || $user->hasRole('administrator')) {
      return $this->redirect(
        'site_admin_edit.form',
        ['adminUserId' => $user->id()]
      );
    } else {
        $entity = \Drupal::entityTypeManager()->getStorage('user')->load($user->id());
        $formObject = \Drupal::entityTypeManager()->getFormObject('user', 'default')->setEntity($entity);
